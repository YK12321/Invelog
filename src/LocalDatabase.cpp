...
bool LocalDatabase::connect() {
    ...
    try {
        ...
        std::vector<std::string> subdirs = {
            "item_types", "items", "containers", "locations", 
...
bool LocalDatabase::saveItemType(std::shared_ptr<ItemType> itemType) {
    if (!connected_ || !itemType) return false;
    
    try {
        json j;
        j["id"] = itemType->getId().toString();
        j["name"] = itemType->getName();
        j["description"] = itemType->getDescription();
        j["specifications"] = itemType->getSpecifications();
        j["manufacturer"] = itemType->getManufacturer();
        j["part_number"] = itemType->getPartNumber();
        
        if (itemType->getCategory()) {
            j["category_id"] = itemType->getCategory()->getId().toString();
        }
        
        std::string filePath = getFilePath("item_types", itemType->getId());
        ...
        std::cerr << "Error saving item type: " << e.what() << std::endl;
        return false;
    }
}

std::shared_ptr<ItemType> LocalDatabase::loadItemType(const UUID& id) {
    if (!connected_) return nullptr;
    
    try {
        std::string filePath = getFilePath("item_types", id);
        ...
        UUID itemTypeId(j["id"].get<std::string>());
        ...
        auto itemType = std::make_shared<ItemType>(itemTypeId, name, category, description,
                                                    specifications, manufacturer, partNumber);
        
        return itemType;
    } catch (const std::exception& e) {
        std::cerr << "Error loading item type: " << e.what() << std::endl;
        return nullptr;
    }
}

bool LocalDatabase::deleteItemType(const UUID& id) {
    ...
    try {
        ...
        std::cerr << "Error deleting item type: " << e.what() << std::endl;
        return false;
    }
}

std::vector<std::shared_ptr<ItemType>> LocalDatabase::loadAllItemTypes() {
    std::vector<std::shared_ptr<ItemType>> itemTypes;
    if (!connected_) return itemTypes;
    
    try {
        std::string dirPath = dataDirectory_ + "/item_types";
        ...
        std::cerr << "Error loading all item types: " << e.what() << std::endl;
    }
...
bool LocalDatabase::saveItem(std::shared_ptr<Item> item) {
    ...
    try {
        ...
        // ItemType reference (NEW)
        if (item->getItemType()) {
            j["item_type_id"] = item->getItemType()->getId().toString();
        }
        
        // Individual tracking fields (NEW)
        j["individual_notes"] = item->getIndividualNotes();
        j["serial_number"] = item->getSerialNumber();
        j["created_by"] = item->getCreatedBy();
        j["created_at"] = timeToString(item->getCreatedAt());
        
        if (item->getOriginLocation()) {
            j["origin_location_id"] = item->getOriginLocation()->getId().toString();
        }
...
std::shared_ptr<Item> LocalDatabase::loadItem(const UUID& id) {
    ...
    try {
        ...
        UUID itemId = UUID::fromString(j["id"].get<std::string>());
        
        // Check if this is a new ItemType-based item or legacy item
        std::shared_ptr<Item> item;
        
        if (j.contains("item_type_id")) {
            // NEW: ItemType-based item
            UUID itemTypeId(j["item_type_id"].get<std::string>());
            auto itemType = loadItemType(itemTypeId);
            
            if (!itemType) {
                std::cerr << "Warning: ItemType " << itemTypeId.toString() 
                         << " not found for item " << itemId.toString() << std::endl;
                // Fall back to legacy loading if ItemType not found
                goto legacy_load;
            }
            
            std::string individualNotes = j.value("individual_notes", "");
            std::string serialNumber = j.value("serial_number", "");
            std::string createdBy = j.value("created_by", "system");
            std::chrono::system_clock::time_point createdAt = std::chrono::system_clock::now();
            
            if (j.contains("created_at")) {
                createdAt = stringToTime(j["created_at"].get<std::string>());
            }
            
            // Use deserialization constructor with UUID
            item = std::make_shared<Item>(itemId, itemType, individualNotes, 
                                          serialNumber, createdBy, createdAt);
            
            // Load origin location
            if (j.contains("origin_location_id")) {
                UUID originLocationId(j["origin_location_id"].get<std::string>());
                auto originLocation = loadLocation(originLocationId);
                if (originLocation) {
...
legacy_load:
            // LEGACY: Traditional item with name, quantity, etc.
            std::shared_ptr<Category> category;
            if (j.contains("category_id")) {
                UUID categoryId = UUID::fromString(j["category_id"].get<std::string>());
                category = loadCategory(categoryId);
            }
...
std::shared_ptr<Container> LocalDatabase::loadContainer(const UUID& id) {
    ...
    try {
        ...
        UUID containerId = UUID::fromString(j["id"].get<std::string>());
...
std::shared_ptr<Location> LocalDatabase::loadLocation(const UUID& id) {
    ...
    try {
        ...
        UUID locationId = UUID::fromString(j["id"].get<std::string>());
...
std::shared_ptr<Project> LocalDatabase::loadProject(const UUID& id) {
    ...
    try {
        ...
        UUID projectId = UUID::fromString(j["id"].get<std::string>());
...
std::shared_ptr<Category> LocalDatabase::loadCategory(const UUID& id) {
    ...
    try {
        ...
        UUID categoryId = UUID::fromString(j["id"].get<std::string>());
