...
public:
    ...
    // ItemType management (NEW - for individual item tracking)
    std::shared_ptr<ItemType> createItemType(const std::string& name,
                                              std::shared_ptr<Category> category,
                                              const std::string& description = "",
                                              const std::string& specifications = "",
                                              const std::string& manufacturer = "",
                                              const std::string& partNumber = "");
    bool deleteItemType(const UUID& itemTypeId);
    std::shared_ptr<ItemType> getItemType(const UUID& itemTypeId);
    std::vector<std::shared_ptr<ItemType>> getAllItemTypes();
    std::vector<std::shared_ptr<ItemType>> searchItemTypes(const std::string& query);
    
    // Individual item instance creation (NEW)
    std::shared_ptr<Item> createItemInstance(std::shared_ptr<ItemType> itemType,
    ...
    // Grouping and counting by type (NEW)
    std::vector<std::shared_ptr<Item>> getItemsByType(const UUID& itemTypeId);
    int getItemCountByType(const UUID& itemTypeId);
    
    // Item management (LEGACY - maintained for backward compatibility)
...
private:
    ...
    // Cached data
    std::vector<std::shared_ptr<ItemType>> itemTypes_;  // NEW
