...
public:
    struct APIConfig {
        ...
        std::string itemTypesEndpoint = "/item-types";
    ...
    // ItemType operations
    bool saveItemType(std::shared_ptr<ItemType> itemType) override;
    std::shared_ptr<ItemType> loadItemType(const UUID& id) override;
    bool deleteItemType(const UUID& id) override;
    std::vector<std::shared_ptr<ItemType>> loadAllItemTypes() override;
