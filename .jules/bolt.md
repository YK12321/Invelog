...
## 2024-08-03 - Optimizing large data aggregations
**Learning:** Fetching an unbounded number of items with `Find()` to aggregate them in Go creates excessive memory allocations and parsing overhead.
**Action:** Push calculations to the database using `Select("COALESCE(SUM(CASE...))")` and `Scan()` to get the result without loading individual records into memory.
